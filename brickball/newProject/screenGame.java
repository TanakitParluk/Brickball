...
public class screenGame extends JPanel implements MouseListener, Runnable{
	...
	Image map = new ImageIcon("images/bg-body.jpg").getImage();
	Image map2 = new ImageIcon("images/wallpaper.png").getImage();
	Shuriken[] arrShu = new Shuriken[50];
	Thread t = new Thread(this);
	public Image shuriken = new ImageIcon("images/blueball.png").getImage();
	public Image shuriken2 = new ImageIcon("images/beachball.png").getImage();
	public Image brickPic = new ImageIcon("images/hole.png").getImage();
	public Image brickPic2 = new ImageIcon("images/brick.jpg").getImage();
	int xNew = 0;
	int yNew = 0;
	int change = 0;
	boolean flagShoot = false; //status shoot
	int count = 0;
	...
	public void checkStat(){		//checking status game after shooting
		boolean flag = true;
		boolean flag2 = false;
			for(int i = 0; i < 12; i++){
				for(int j = 0; j < 14; j++){
	...
	public void paintComponent(Graphics g) {			//draw overall picture
		...
		if(change%2==1) {
			g.drawImage(map2, 0, 0, getWidth(), getHeight(), this);
			for (int i = 0; i < 12; i++) {
				for (int j = 0; j < 14; j++) {
					if (brick[i][j] != 0) {
						g.drawImage(brickPic2, 0 + (j * 50), 0 + (i * 50), 50, 50, this);
						g.drawString("" + brick[i][j], (0 + (j * 50) + 15), (0 + (i * 50) + 30));
					}
				}
			}
		}
		synchronized (arrShu) {					//about shooter
			for(int i = 0; i < arrShu.length; i++){
				g.drawImage(arrShu[i].shuriken, arrShu[i].x, arrShu[i].y, 15, 15, this);
				if(arrShu[i].flag){
					if(xNew == 0){
						xNew = arrShu[i].x;
